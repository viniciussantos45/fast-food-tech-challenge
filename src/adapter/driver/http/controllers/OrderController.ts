...
export async function createOrder(request: FastifyRequest<{ Body: OrderCreateDto }>, reply: FastifyReply): Promise<void> {
  const { customer, combos } = request.body

  const customerCpf = new CPF(customer.getCpf()!)
  const productIdsByCombo = combos.map((combo) => ({
    comboId: combo.getId(),
    productIds: combo.getProductsIds()
  }))

  orderService.createOrder({ productIdsByCombo, customerCpf })
